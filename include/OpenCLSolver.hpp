...
private:
    ...
    cl_mem bufKappaFace = nullptr;
    cl_mem bufSource = nullptr;
...
public:
    ...
    bool update_source(Mesh* mesh) {
        if (!available) return false;
        cl_int err = clEnqueueWriteBuffer(queue, bufSource, CL_TRUE, 0,
                                         sizeof(float_t) * mesh->get_ncells(),
                                         mesh->source.data(), 0, nullptr, nullptr);
    ...
    bool init_buffers(Mesh* mesh) {
        ...
        bufKappaFace = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                  sizeof(float_t) * mesh->kappa_face.size(), 
                                  mesh->kappa_face.data(), &err);
        if (err != CL_SUCCESS) return false;

        bufSource = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                sizeof(float_t) * ncells, 
                                mesh->source.data(), &err);
        if (err != CL_SUCCESS) return false;
    ...
    bool step_device(int_t ncells, float_t alpha, float_t dt) {
        ...
        err |= clSetKernelArg(kernel, 5, sizeof(cl_mem), &bufKappaFace);
        err |= clSetKernelArg(kernel, 6, sizeof(cl_mem), &bufSource);
        err |= clSetKernelArg(kernel, 7, sizeof(cl_int), &ncells);
...
private:
    void build_kernel_source() {
        ...
        __kernel void heat_fvm(
            ...
            __global const float* kappa_face,
            ...
            const float dt)
        {
            ...
            for (int k = 0; k < 4; ++k) {
                ...
                // Поток через грань с неоднородным коэффициентом
                float kappa = kappa_face[face_idx];
                float grad_T = (T_neighbor - T_owner) / face.distance;
                float flux = kappa * face.area * grad_T;
                
                flux_sum += flux;
            }
            
            // Обновление температуры с учетом источника
            T_next[cell] = T_curr[cell] + dt * (flux_sum / volumes[cell] + source[cell]);
        }
        )CLC";
    }
